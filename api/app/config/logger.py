...
class CustomizeLogger:
    ...
    def make_logger(cls):
        logger = cls.customize_logging(
            os.getenv('LOGGER_PATH') ,
            level=os.getenv('LOGGER_LEVEL'),
            retention=os.getenv('LOGGER_RETENTION'),
            rotation=os.getenv('LOGGER_ROTATION'),
    ...
    def customize_logging(cls,
        ...
        logger.remove()
        logger.level("DETAIL", no=7, color="<white>") # For unstructured logging
        logger.add(
            sys.stdout,
            enqueue=True,
            backtrace=True,
